module namespace s2i='http://code.google.com/p/xrx/s2i';
...
(: Todo
  Check to see if minOccurs is greater than 1.
  Enumerations
  References
...
declare function s2i:process-type($type-name)   {
(: just simple types - xs namespace :)
   if ($type-name = ("xs:string","xs:NMTOKEN","xs:NCName"))
   then 'string'
   
   else if ($type-name="xs:integer")
   then '5'
   
   else if ($type-name="xs:anyURI")
   then 'http://www.example.com'
   
   else if ($type-name="xs:decimal")
   then '5.5'
   
   else  if ($type-name="xs:boolean")
   then 'true'
   
   else  if ($type-name="xs:short")
   then '32767'
   
   else  if ($type-name="xs:byte")
   then '126'
   
   else  if ($type-name="xs:unsignedByte")
   then '255'
   
   else if ($type-name="xs:date")
   then '2009-01-01'
   
   else if ($type-name="xs:time")
   then '24:12:60'
   
   else if ($type-name="xs:dateTime")
   then '2009-01-01T24:12:60'
   
   else ('no type found')
   (: TODO add the other XML Schema types: byte, short, int, positiveInteger etc. :)
};
